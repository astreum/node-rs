...
impl Block {
    ...
    pub fn apply(&self, accounts: &mut Accounts, latest_block: &Block) -> Result<(), Box<dyn Error>> {

        let solar_price = nova::next_solar_price(&latest_block);

        let validator = nova::validator_selection(accounts, latest_block, &self.time);
        ...
        .all(|&x| x) {
            ...
            for tx in &self.transactions {
                ...
                match tx.apply(&accounts.store, &mut changed_accounts, &solar_price) {
            ...
            if [receipts_hash == self.receipts_hash,  accounts_hash == self.accounts_hash].iter().any(|&x| x == false) {
